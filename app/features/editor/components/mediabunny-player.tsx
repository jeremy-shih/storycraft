...
export function MediabunnyPlayer({
    ...
    const [activeVideoElement, setActiveVideoElement] = useState<
        "main" | "next"
    >("main");
    ...
    const getCurrentVideoClip = useCallback(
        (time: number): { item: TimelineItem; clipTime: number } | null => {
    ...
    const getNextVideoClip = useCallback(
        (currentClipId: string): TimelineItem | null => {
            const videoLayer = layers.find((l) => l.type === "video");
            if (!videoLayer) return null;

            const currentClip = videoLayer.items.find(
                (item) => item.id === currentClipId,
            );
            ...
            const sortedItems = [...videoLayer.items]
                .filter(
                    (item) =>
                        item.content &&
                        item.startTime >=
                            currentClip.startTime + currentClip.duration - 0.1,
                )
    ...
    const getActiveVideo = useCallback((): HTMLVideoElement | null => {
        return activeVideoRef.current === "main"
            ? videoRef.current
    ...
    const stopAllAudio = useCallback(
        (shouldRecreateContext: boolean = false) => {
    ...
    const startAudioClip = useCallback(
        async (clip: {
            item: TimelineItem;
            clipTime: number;
            type: "voiceover" | "music";
            ...
            if (
                activeSourcesRef.current.has(clipId) ||
                startingAudioClipsRef.current.has(clipId) ||
            ...
            try {
                ...
                const startOffset = Math.max(
                    0,
                    Math.min(clip.clipTime, buffer.duration - 0.01),
                );
    ...
    const syncAudioState = useCallback(
        (timelineTime: number) => {
            ...
            activeAudioClips.forEach((clip) => {
                if (
                    !activeSourcesRef.current.has(clip.item.id) &&
                    !startingAudioClipsRef.current.has(clip.item.id)
                ) {
                    startAudioClip(clip);
                }
            });
    ...
    const startPlaybackLoop = useCallback(() => {
        ...
        const tick = () => {
            ...
            if (currentClip && video) {
                ...
                if (videoTime >= clipEndTime - 0.05) {
                    // Small buffer to prevent stuttering
            ...
            if (clipAtTime && video) {
                ...
                if (
                    ...
                    currentClipRef.current.id !== clipAtTime.item.id
                ) {
                    ...
                    const trimStart =
                        (clipAtTime.item.metadata?.trimStart as number) || 0;
                    currentClipRef.current = {
                        id: clipAtTime.item.id,
                        startTime: clipAtTime.item.startTime,
                        endTime:
                            clipAtTime.item.startTime +
                            clipAtTime.item.duration,
                    ...
                    const preloadVideo = getPreloadVideo();
                    if (
                        preloadedClipIdRef.current === clipAtTime.item.id &&
                        preloadVideo &&
                        preloadVideo.readyState >= 2
                    ) {
                ...
                if (activeVideoIsPlaying && currentClipRef.current) {
                    // Calculate timeline time from video time: startTime + (videoCurrentTime - trimStart)
                    timelineTime =
                        currentClipRef.current.startTime +
                        (activeVideo.currentTime -
                            currentClipRef.current.trimStart);
    ...
    useEffect(() => {
        ...
        const hasNewContent = videoUrls.some(
        ...
        if (sortedClips.length > 0 && videoRef.current) {
            ...
            if (
                ...
                !videoRef.current.src.includes(
                    firstClip.content.split("/").pop() || "",
                )
            ) {
            ...
            if (sortedClips.length > 1 && nextVideoRef.current) {
                ...
                if (
                    secondClip.content !== firstClip.content &&
                    !preloadedUrlsRef.current.has(secondClip.content)
                ) {
            ...
            >
                <video
                    ...
                    className={`absolute inset-0 h-full w-full object-contain ${
                        showBlackScreen || activeVideoElement !== "main"
                            ? "hidden"
                            : ""
                    }`}
                ...
                <video
                    ref={nextVideoRef}
                    className={`absolute inset-0 h-full w-full object-contain ${
                        showBlackScreen || activeVideoElement !== "next"
                            ? "hidden"
                            : ""
                    }`}
                    onLoadedMetadata={handleLoadedMetadata}
                        ...
                        <div className="flex flex-col items-center gap-3">
                            <div className="flex gap-1">
                                {[0, 1, 2].map((i) => (
                                    <div
                                        key={i}
                                        className="h-8 w-2 animate-bounce rounded-full bg-blue-500"
                                        style={{
                ...
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 transform">
                    <Button
                        ...
                        size="icon"
                        onClick={onPlayPause}
                        className="bg-black/50 transition-colors hover:bg-green-500 hover:text-white"
                    >
                        {isPlaying ? (
                            <svg
